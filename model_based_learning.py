...
def update_mdp_model_with_history(counts, rewards, history):
	...
	for state, action, reward, next_state, done in history:
		r = rewards[state][action][next_state]
		n = counts[state][action][next_state]
		rewards[state][action][next_state] = (r * n + reward) / (n + 1)
		counts[state][action][next_state] = 1
		if done:
			terminal_state.add(next_state)

	for s in terminal_state:
		for a in range(nA):
			counts[s][a][s] = 1
			for ss in range(nS):
				rewards[s][a][ss] = 0.0
...
def learn_with_mdp_model(env, num_episodes=5000, gamma=0.95, e=0.8, decay_rate=0.99):
	...
	for ep in range(num_episodes):
		...
		while not done:
			...
			state = next_state
		update_mdp_model_with_history(counts, rewards, history)
		P = counts_and_rewards_to_P(counts, rewards)
		_, policy = value_iteration(P, env.nS, env.nA, gamma, 30, 1e-3)
